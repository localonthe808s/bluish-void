...
MAG_LIMIT = 6.0      # naked-eye depth — the whole sky gets at least this
LAT_MIN = -85.0      # wide windows show far beyond the square viewBox —
                     # keep nearly the whole sky so corners never go empty

# The azimuthal-equidistant projection dilutes the southern sky: screen
# density scales with sin(θ)/θ (θ = 90°−β), ~3× thinner at β=−60° than on
# the ecliptic, so wide frames looked empty at the sides. Compensate with
# REAL fainter stars (stars.8.json, mag ≤ 8.5) — the magnitude cut ramps
# deeper as the projection stretches. Mobile skips the >6.0 tier at draw
# time, so the phone's node count is unchanged.
def mag_limit(beta):
    if beta >= 10:
        return MAG_LIMIT
    t = min(1.0, (10.0 - beta) / 95.0)   # 0 at β=+10° … 1 at β=−85°
    return MAG_LIMIT + 1.5 * t           # 7.5 at the far southern edge
...
#     curl -sL -o /tmp/stars8.json \\
#       https://raw.githubusercontent.com/ofrohn/d3-celestial/master/data/stars.8.json
stars = []
for f in json.load(open('/tmp/stars8.json'))['features']:
    mag = f['properties']['mag']
    if mag > 7.5:                        # cheap pre-cut before the trig
        continue
    ra, dec = f['geometry']['coordinates']
    ra = ra % 360
    lam, beta = equ_to_ecl(ra, dec)
    if beta < LAT_MIN or mag > mag_limit(beta):
        continue
    try:
...
out.write_text(
    ...
    '   Stars: HYG database via d3-celestial stars.8.json (mag ≤ %.1f on the ecliptic,\n'
    '   ramping to 7.5 in the projection-stretched south; ecl lat ≥ %.0f°).\n'
